...
int main(int argc, char **argv)
{
  ...
  std::string ip_address="127.0.0.1", port_to_unity;
  float offset_x=0, offset_y=0;
  std::string pose_topic;
  
  fla_utils::SafeGetParam(n, "w_to_unity/port_to_unity", port_to_unity);
  fla_utils::SafeGetParam(n, "w_to_unity/pose_topic", pose_topic);

  ROS_INFO("This node enables send communication to Unity\n");

  UDPPoseStreamer streamer(ip_address, port_to_unity, offset_x, offset_y, pose_topic);
