...
int main(int argc, char *argv[])
{
  ...
  std::string port_from_unity, name;
  fla_utils::SafeGetParam(n, "unity_ros/port_from_unity", port_from_unity);
  fla_utils::SafeGetParam(n, "unity_ros/name", name);

  ROS_INFO("Starting TCPStreamReader");
  
  TCPStreamReader stream_reader("127.0.0.1", port_from_unity);
  ...
  stream_parsers[UnityMessageType::UNITY_STATE] = std::make_shared<TrueStateParser>(name);
