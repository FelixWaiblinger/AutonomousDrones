...
class StateEstimateCorruptorNode {
 public:
	StateEstimateCorruptorNode() {
		...
		fla_utils::SafeGetParam(pnh, "true_pose_topic", true_pose_topic);
		fla_utils::SafeGetParam(pnh, "true_twist_topic", true_twist_topic);
		fla_utils::SafeGetParam(pnh, "est_pose_topic", est_pose_topic);
		fla_utils::SafeGetParam(pnh, "est_twist_topic", est_twist_topic);
		fla_utils::SafeGetParam(pnh, "est_state_topic", est_state_topic);
		fla_utils::SafeGetParam(pnh, "body", body);

		// Subscribers
		pose_sub     = nh.subscribe(true_pose_topic, 1, &StateEstimateCorruptorNode::OnPose, this);
		velocity_sub = nh.subscribe(true_twist_topic, 1, &StateEstimateCorruptorNode::OnVelocity, this);

		// Publishers
		corrupted_pose_pub     = nh.advertise<geometry_msgs::PoseStamped>(est_pose_topic, 1);
		corrupted_velocity_pub = nh.advertise<geometry_msgs::TwistStamped>(est_twist_topic, 1);
		corrupted_state_pub    = nh.advertise<nav_msgs::Odometry>(est_state_topic, 1);
 ...
 private:
	...
	void PublishCorruptedState(geometry_msgs::TwistStamped const& corrupt_twist) {
		...
		corrupted_state.child_frame_id = body; // quadrotor/body
	...
	void PublishBodyTransform(geometry_msgs::PoseStamped const& pose) {
		...
		transformStamped.child_frame_id = body; // quadrotor/body
	...
	std::string true_pose_topic, true_twist_topic, est_pose_topic, est_twist_topic, est_state_topic;
